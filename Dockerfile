...
RUN wget https://repo1.maven.org/maven2/com/github/magese/ik-analyzer/7.7.1/ik-analyzer-7.7.1.jar -O /opt/solr/server/solr-webapp/webapp/WEB-INF/lib/ik-analyzer-7.7.1.jar && \
    ...
    wget https://downloads.apache.org/logging/log4j/2.16.0/apache-log4j-2.16.0-bin.tar.gz -O /tmp/log4j.tgz && \
    cd /tmp/ && \
    tar xzvf log4j.tgz && \
    cp /tmp/apache-log4j-2.16.0-bin/log4j-core-2.16.0.jar /tmp/apache-log4j-2.16.0-bin/log4j-api-2.16.0.jar /tmp/apache-log4j-2.16.0-bin/log4j-1.2-api-2.16.0.jar /tmp/apache-log4j-2.16.0-bin/log4j-slf4j-impl-2.16.0.jar /opt/solr/server/lib/ext/ && \
    rm -rf  /tmp/apache-log4j-2.16.0-bin && rm /tmp/log4j.tgz && \
